...
from Models.MiniAttention import FullSimpleTransformer, FullDualTransformer
...
from dataHandlers.SingleSymbolHandler import SequenceDataset, SeparateSequenceDataset, TwoDayDataset


def two_day_attention_test():
    ...
    model = FullDualTransformer(dim_in=in_size, dim_attn=64, attention_depth=1, dim_out=9, mlp_depth=3,  rotary_emb=True, dropout=0.3).to(device)
    ...
    trainDataset = TwoDayDataset(data_type='train', path="./", start_date=680, end_date=1520, out_size=out_size, in_size=in_size, device=device, symbol_id=0)
    evalDataset = TwoDayDataset(data_type='eval', path="./", start_date=1520, end_date=1699, out_size=out_size, in_size=in_size, device=device, symbol_id=0)
