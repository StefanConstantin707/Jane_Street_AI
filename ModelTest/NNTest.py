...
from dataHandlers.PartialDataHandler import RowSamplerDatasetPD, SingleRowPD, SingleRowNoisePD
...
def nn_test_noise():
    ...
    model = SimpleNN(input_size=in_size, hidden_dim=256, output_size=out_size, num_layers=5, dropout_prob=0.0, noise=noise).to(device)
    total_params = sum(p.numel() for p in model.parameters())
    print(f"Total number of parameters: {total_params}")
